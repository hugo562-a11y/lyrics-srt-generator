        ...
        try:
            ...
            if not lyrics:
                if reference and intro_filter:
                    ...
                    onset_raw, _ = model.transcribe(str(source_path), language=None if language == "auto" else language, vad_filter=True, condition_on_previous_text=False, beam_size=5, vad_parameters=vad_params, initial_prompt=prompt_text or None)
                    onset_segments = normalize_lyrics(list(onset_raw))
                    if onset_segments:
                        vocal_onset = onset_segments[0].start
                self.events.put(("status", "正在分析音訊與逐字時間點，請稍候…"))
                raw, _ = model.transcribe(str(source_path), language=None if language == "auto" else language, vad_filter=not bool(reference), condition_on_previous_text=False, beam_size=5, word_timestamps=precise, vad_parameters=vad_params, no_speech_threshold=no_speech, initial_prompt=prompt_text or None)
