...
- 空白鍵＝播放／暫停（全域快捷鍵）。已在 `LyricsSrtApp.__init__` 用 `bind_class("TButton"/"TCheckbutton", "<space>", ...break...)` 擋掉按鈕與勾選框原本的空白鍵行為，避免誤觸目前焦點所在的按鈕；焦點在文字輸入框（表格內雙擊編輯用的 `ttk.Entry`／`ttk.Combobox`）時空白鍵仍正常輸入，不會觸發播放。
