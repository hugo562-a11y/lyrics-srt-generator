...
最後更新：2026-07-13（暗色主題、修正空白鍵與點擊自動播放）
...
- 空白鍵＝播放／暫停（全域快捷鍵，`_on_space_key`）。焦點在文字輸入框（表格內雙擊編輯用的 `ttk.Entry`／`ttk.Combobox`）時空白鍵仍正常輸入，不會觸發播放。
  - **踩過的坑**：一開始用 `bind_class("TButton"/"TCheckbutton", "<space>", lambda _e: "break")` 擋掉按鈕的預設空白鍵行為，結果 `"break"` 會連 bindtags 後面的 `all`（全域）都一併擋掉；而 ttk 按鈕點擊後預設會把鍵盤焦點留在該按鈕上，導致點過任何按鈕之後空白鍵完全沒反應。修法：改成 `lambda _e: None`（不回傳 break），因為 `bind_class` 本來就會整個取代該 class 原本的空白鍵綁定，不需要再額外 break 去阻止按鈕自己觸發。
- 暗色主題：`ttk.Style` 改用 `theme_use("clam")`（唯一能完全套用自訂顏色的內建主題，預設 `vista` 主題大多顏色設定會被忽略），並在 `_build_ui` 開頭統一設定按鈕／輸入框／下拉選單／表格／捲軸／進度條的深色配色（色票常數集中在檔案開頭 `DARK_*`／`WAVE_*` 等）。另外用 `DwmSetWindowAttribute`（`_apply_dark_titlebar`）讓 Windows 10/11 的原生標題列也一併變暗，不支援的系統會靜默略過。
